...
string StringTools::GetLongWordsDate(const string &yyyymmdd,char lang) {
   string lcldate=yyyymmdd;
   string year=GetFirstChunkAndDeleteFromLine(lcldate,'-');
   string month=GetFirstChunkAndDeleteFromLine(lcldate,'-');
   ...
   switch ( lang ) {
      ...
      default :
         lngdt=yyyymmdd;
         std::cout << "Language not implemented!" << '\n';
         std::cout << __FILE__ << ", fnc: " << __FUNCTION__ << ", line: " << __LINE__ << '\n';
         break;
   }
   return lngdt;
