...
int FileUtils::CountColumnsInCSVFile(const string &fnam,const char sep) {
   string line;
   ifstream ifil;
   ifil.open(fnam.c_str(),std::ios::in);
   DiscardComments(ifil);
   ...
   line+=sep;
   int numtabs=0;
   for (size_t i=0; i<line.length(); i++) {if (line[i]==sep) { ++numtabs; }}
   return numtabs;
}
...
vector<vector<string> > FileUtils::ReadStringsMultiColumsFromCSV(\
      string fname, const char sep) {
   ifstream ifil(fname);
   size_t cols=size_t(CountColumnsInCSVFile(fname,sep));
   size_t rows=size_t(CountNumberOfNonCommentLines(fname));
   vector<vector<string> > data(rows);
   vector<string> tmp(cols);
   string line,word;
   for ( size_t i=0 ; i<rows ; ++i ) {
      getline(ifil,line);
      for ( size_t j=0 ; j<(cols-1) ; ++j ) {
         word=StringTools::GetFirstChunkAndDeleteFromLine(line,sep);
         StringTools::RemoveSpacesLeftAndRight(word);
         StringTools::RemoveRedundantSpaces(word);
         data[i].push_back(word);
      }
      StringTools::RemoveRedundantSpaces(line);
      StringTools::RemoveSpacesLeftAndRight(line);
...
bool FileUtils::HasWindowsNewLines(const string &fname) {
   char t;
   bool res=false;
   ifstream ifil(fname);
   while ( !(ifil.eof()) ) {
      ifil.get(t);
      if ( t=='\r' ) { res=true; break; }
   }
   ifil.close();
   return res;
}
